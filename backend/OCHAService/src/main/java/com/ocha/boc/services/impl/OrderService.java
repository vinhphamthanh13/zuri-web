...
import com.ocha.boc.request.PageRequest;
...
//    }
//
    public OrderResponse getOrdersByCuaHangId(PageRequest pageRequest) {
        OrderResponse response = new OrderResponse();
        response.setSuccess(Boolean.FALSE);
        response.setMessage(CommonConstants.GET_ORDERS_BY_CUAHANGID_FAIL);
        String[] sortSplit = pageRequest.getSort().split(",");
        try {
            Page<Order> orders = orderRepository.findAll(new org.springframework.data.domain.PageRequest(pageRequest.getPage(), pageRequest.getSize(),
